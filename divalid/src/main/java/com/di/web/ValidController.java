...
//			pubyear = "2014";
			...
			if("PDB".equals(hybaseKey)){
				...
				// {total=6184, docInfo={pd=20141230}}
					...
					long count = 0;
					if(!Strings.isNullOrEmpty(mongoSearchkey)){
						count = mongoController.count(mongoGroupkey+";"+mongoSearchkey, keyformat+";"+mongoSearchvalue);
					}else{
						count = mongoController.count(mongoGroupkey, keyformat);
					}
					build.append(keyformat + "\t mongovalue:\t " + count);
					build.append("\t hybasevalue:\t " + hybasevalue );
					if(hybasevalue.longValue() != count){
		...
		public String count(String key, String strWhere, String top) throws Exception {
			DataBaseFactory _factory = DataBaseFactory.newInstance(Constants.DB_TYPE_HYBASE, hybaseConfig.hybasehost,
					hybaseConfig.hybaseport, hybaseConfig.hybaseuser, hybaseConfig.hybasepassword);
			...
			_searchParams.setProperty("search.syntax.name", "trs");//设置默认检索语法
			...
			try {
				...
				strWhere = "((ASO=data()))";
				_rs = _con.executeSelect(trsHybaseTable, strWhere, 0, 10, _searchParams);
				_iSize = _rs.getNumFound();
				build.append(strWhere + ":" + _iSize + "\r\n");
			} catch (Exception e) {
				e.printStackTrace();
				throw e;
			}finally{
