...
public class CKMController {
	...
	public @ResponseBody DEM TRSPatentCluTestCaseNew(String queryexpr){
		...
		return TRSPatentCluTestCase(queryexpr,5,5,"",5);
	  }
	
	public @ResponseBody DEM TRSPatentCluTestCase(String queryexpr,int mincls,int maxcls,String StopWord,int Keywordnum){
...
//            Thread.sleep(1000);
		...
		Result result = _factory.getQuery().select(hybaseConfig.tablename_pat, readColumn, null, querynos, null, start, recordNum, params);
	...
	public @ResponseBody List<Serie> testxyz(String query){
			...
			for(int i = 0;i<dem.CL_clusters.size();i++){
				...
				serie.center[0] = clu.cx;serie.center[1] = clu.cy;serie.center[2] = clu.cz;
				int size = clu.CL_patent_table.size();
				float[][] data = new float[size][3];
				for(int j = 0; j < size ;j++){
					CL_patent patent  = clu.CL_patent_table.get(j);
	...
	public void  jsonpstd(HttpServletRequest request,HttpServletResponse response) throws IOException{
	      ...
	      response.setContentType("text/plain");  
	        ...
	        if(request.getParameter("mincls") != null){
		        ...
		        value = maper.writeValueAsString(TRSPatentCluTestCase(query,mincls,maxcls,StopWord,Keywordnum));
	        }else{
