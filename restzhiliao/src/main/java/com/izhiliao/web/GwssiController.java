...
import com.izhiliao.util.Category;
import com.izhiliao.util.ExcelUtil;
...
public class GwssiController {
	...
	public List<Category> category(String key, String strWhere, String top) throws Exception {
		// public Map<String, Map<String, Long>> category(String key,String
		// strWhere,String top) throws Exception {
	...
	String[] orderarray = {"RELEVANCE", "+RELEVANCE", "AD", "+AD", "AD", "+AD", "PD", "+PD", "EPRD", "+EPRD", "INCO",
			"+INCO", "IPCSCC", "+IPCSCC", "CLN", "+CLN", "DEPC", "+DEPC", "DC", "+DC"};
	String[] query = {"PD=2014", "PDT=1", "PDT=2", "PDT=3", "IPCS=A", "IPCS=B", "IPCS=C", "IPCS=D", "IPCS=E", "IPCS=F",
			"IPCS=G", "IPCS=H", "AD=2013"};
	...
	public void category(String strWhere) throws Exception {
		...
		WritableWorkbook book = Workbook.createWorkbook(new File(hybaseConfig.testdatapath + "CategoryResult-"
				+ strWhere.replace("=", "") + ".xls"));
		// 第0页作为统计页
	...
	public void ExcuteQueryFromExcel() throws Exception {
		...
		WritableWorkbook bookCreate = Workbook.createWorkbook(new File(hybaseConfig.querydatapath
				+ "CategoryResult-Modify.xls"), book);
		// 第0页作为统计页
		...
		for (WritableSheet sheet : bookCreate.getSheets()) {
			...
			// 如果符合模板
			if ("TableName".equals(sheet.getCell(0, 0).getContents())
					&& "Key".equals(sheet.getCell(0, 1).getContents())
					&& "Query".equals(sheet.getCell(1, 1).getContents())
					&& "PRECount".equals(sheet.getCell(2, 1).getContents())) {
				{
					...
					for (int row = 2; row < sheet.getRows(); row++) {
						...
						try {
							...
							// TimeCost
							ExcelUtil.createSheetColumn(sheet, 5, row,
	...
	public void ExcuteCategoryFromExcel() throws Exception {
		...
		WritableWorkbook bookCreate = Workbook.createWorkbook(new File(hybaseConfig.querydatapath
				+ "CategoryResult2-CategoryModify.xls"), book);
		// WritableSheet sheet1 = bookCreate.getSheet("1000-10000");
		for (WritableSheet sheet : bookCreate.getSheets()) {
			// 如果符合模板
			if ("Key".equals(sheet.getCell(0, 0).getContents()) && "Query".equals(sheet.getCell(1, 0).getContents())
					&& "PRECount".equals(sheet.getCell(2, 0).getContents())) {
