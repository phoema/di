 ...
 */
public class Actions_GwssiCategory {
	private String url =  "http://192.168.13.123:8085/ckmtest/gwssi/category";
	...
	public void readLR(String strWhere,String key) {
		// lr参数设置
		...
		//String url = "http://192.168.13.123:8085/ckmtest/gwssi/category?key=%s&&strWhere=%s&&top=%s";
		...
		try {
			stat = lrapi.web.submit_data("", "Action=" + url, new String[]{
					...
					"Name=key", "Value="+key, lrapi.web.ENDITEM, 
					"Name=strWhere", "Value=" + strWhere,  lrapi.web.ENDITEM,
					"Name=top", "Value=10" ,lrapi.web.ENDITEM, 
					 lrapi.web.LAST});

			totalExecuteTime++;
                        System.out.println(lr.eval_string("<ResponseBody>"));
		} catch (Exception e1) {
